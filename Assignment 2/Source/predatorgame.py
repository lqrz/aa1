...
class PredatorGame():
        ...
        def monteCarloControl(self, discount, epsilon, N, initValue, optimalV=None, optimalPolicy=None, predCoord=None, preyCoord=None):
            # Initialization
            Q = dict()
            Returns = dict()
            probPolicy = dict()
            ...
            for episode in range(N):
                ...
                self.reset()
                episodePredCoord = self.predCoord
                episodePreyCoord = self.preyCoord
                
                # Generate an episode
                episode = self.generateEpisode(probPolicy, predCoord, preyCoord)
                ...
                # Re-do the episode using the optimal policy
                self.reset(episodePredCoord, episodePreyCoord)
                optimalSteps = 0
                while not self.state:
                    self.step(optimalPolicy)
                    optimalSteps = optimalSteps+1
                stepRatios.append(len(episode) * 1.0 / optimalSteps)

                # Calculate root mean square error
                for state in states:
                    Vmc[state] = max(Q[state][a] for a in self.moves)
                if not optimalV == None:
                    RMSs.append(  math.sqrt( sum( 1.0 * (Vmc[state] - optimalV[state])**2 for state in states) / len(Vmc) )  )
                   
            return Q, stepCounts, stepRatios, RMSs

        # Convert a policy to probabilistic form
        def optimalPolicyWithAllActions(self, optimalV, discount):
            states = self.getStates()

            policy = dict()
            for state in states:
                policy[state] = dict()
                policy[state] = self.getAllMaximizingActions(state, discount, optimalV)

            return policy

        # Execultes Off-Policy Monte Carlo Control
        def offPolicyMonteCarloControl(self, discount, epsilon, N, initValue, optimalV=None, optimalPolicy=None, predCoord=None, preyCoord=None):
            # Initialization
            states = self.getStates()
            Q = dict()
            ...
            probPolicy = dict()
            behaviorPolicy = dict()
            policy = dict() # The policy we want to improve
            
            for state in states:
                Q[state] = dict()
                Numerator[state] = dict()
                Denominator[state] = dict()
                behaviorPolicy[state] = dict()
                ...
                for action in self.moves:
                    ...
                    if optimalPolicy[state] == policy[state]:
                        if optimalPolicy[state] == action:
                            behaviorPolicy[state][action] = 0.8
                        else:
                            behaviorPolicy[state][action] = 0.2 / (len(self.moves)-1)
                    else:
                        if optimalPolicy[state] == action or policy[state] == action:
                            behaviorPolicy[state][action] = 0.4
                        else:
                            behaviorPolicy[state][action] = 0.2 / (len(self.moves)-2) # random policy
            ...
            # Percentage of optimal steps taken
            optimalStepRatios = []
            ...
            for episode in range(N):
                ...
                episodePredCoord = self.predCoord
                episodePreyCoord = self.preyCoord
                episode = self.generateEpisode(behaviorPolicy, episodePredCoord, episodePreyCoord)
                ...
                for i, step in enumerate(set(episode[tau:])):
                    state = step[0]
                    action = step[1]
                    # Time of first occurance
                    time = [k for k in range(tau,len(episode)) if step == episode[k]][0]
                    w = np.prod([1/behaviorPolicy[episode[k][0]][episode[k][1]] for k in range(time+1,len(episode)-1)])
                    retrn = sum([(discount ** k) * self.getReward(returnPair[0], returnPair[2]) for k, returnPair in enumerate(episode[time:])])
                    # print [(discount ** k) * self.getReward(returnPair[0], returnPair[2]) for k, returnPair in enumerate(episode[time:])]
                    # print len(episode[time:])
                    # print episode[time:(time+10)]
                    # print retrn
                    # print time
                    # print "****"
                    Numerator[state][action] = Numerator[state][action]+w*retrn
                    # print w
                    # print Numerator[state][action]
                    # print time
                    # print "****"
                    Denominator[state][action] = Denominator[state][action]+w
                    Q[state][action] = Numerator[state][action]/Denominator[state][action]
                for state in self.getStates():
                    # Find the maximizing policy
                    policy[state] = self.getGreedyAction(Q[state])
                #print "Try policy"
                
                # Use the policy to count the number of steps needed
                self.reset(episodePredCoord, episodePreyCoord)
                steps = 0
                while not self.state:
                    policy[self.getState(self.predCoord,self.preyCoord)] = self.getActionEpsilonGreedy(epsilon, Q[self.getState(self.predCoord,self.preyCoord)])
                    self.step(policy)
                    steps = steps+1
                    if steps > 1000:
                        print policy
                        break
                    #sys.stdout.write(str(policy[self.getState(self.predCoord,self.preyCoord)]))
                ...
                while not self.state:
                    self.step(optimalPolicy)
                    optimalSteps = optimalSteps+1
                stepRatios.append(steps * 1.0 / optimalSteps)

                # Calculate the ratio of optimal steps
#                 optimalPolicyWithAllActions = self.optimalPolicyWithAllActions(optimalV, discount)
#                 
#                 optimalStepCount = 0
# 
#                 for i in range(len(episode)):
#                     # If the action taken is optimal, increment the count
#                     if episode[i][1] in optimalPolicyWithAllActions[episode[i][0]]:
#                         optimalStepCount += 1
# 
#                 optimalStepRatios.append(optimalStepCount * 1.0 / len(episode))
            ...
            return Q, stepCounts, stepRatios, optimalStepRatios, RMSs

        #-------------- Temporal Difference Learning Algorithms ------------------------------

        # Executes SARSA
        def sarsa(self, discount, alpha, epsilon, N, initQ, softmax, optimalV=None):
            ...
            return Q, stepCounters, optimalPolicy, RMSs

        # Executes Qlearning
        def Qlearning(self, discount, epsilon, N, alpha, initQ, softmax, optimalV=None):
        ...
        def reset(self, pred=(0,0), prey=(5,5)):
            ...
            if pred is None:
                self.predCoord = (np.random.randint(0, self.boardSize[0]), np.random.randint(0, self.boardSize[1]))
            else:
                self.predCoord = pred

            if prey is None:
                self.preyCoord = (np.random.randint(0, self.boardSize[0]), np.random.randint(0, self.boardSize[1]))
            else:
                self.preyCoord = prey
